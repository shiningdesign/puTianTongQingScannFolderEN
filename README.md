...
Final Notes:
  * as mostly likely some bad guys will improve the virus code for fun, and circle it around for future, you probaly need to keep update the tool to make it up to date without future similar virus, as there may be so many alternative way of writting it, so those Keyword detection method may not one for all.
